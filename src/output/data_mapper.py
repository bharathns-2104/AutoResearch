...
class ReportDataMapper:
    ...
    def _map_executive_summary(self, data):
        return {"summary_text": data.get("executive_summary", "")}
    ...
    def _map_recommendations(self, data):
        return {"recommendations": data.get("final_recommendations", [])}
    ...
    def _map_data_confidence(self, data):
        """
        FIX #6 / #11: consolidation_agent stores confidence under BOTH:
          - data["metadata"]["data_confidence"]  (primary — for this mapper)
          - data["data_quality"]                 (secondary — for direct access)
        Try both; fall back to "Medium" if neither exists.
        """
        # Primary path: metadata.data_confidence (set by consolidation_agent)
        confidence = (data.get("metadata") or {}).get("data_confidence") or {}

        # Secondary path: data_quality (same dict, different key)
        if not confidence:
    ...
    def _map_sources(self, data):
        ...
        for src in raw_sources:
            ...
            if not url:
                continue
            normalised.append({"title": src.get("title", "") or url, "url": url})
        return normalised